...
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content">
    <form id="dataForm" method="POST" action="../../modules/entry/employee_entry.php">
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col-md-4 mb-3"><label class="form-label">Employee ID</label><input type="text" name="emp_id" id="emp_id" class="form-control"></div>
                ...
                <div class="col-md-4 mb-3"><label class="form-label">Name (BN)</label><input type="text" name="name_bn" id="name_bn" class="form-control"></div>
            </div>
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3"><label class="form-label">Mobile <span class="text-danger">*</span></label><input type="text" name="mobile" id="mobile" class="form-control" required></div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3"><label class="form-label">NID <span class="text-danger">*</span></label><input type="text" name="nid" id="nid" class="form-control" required></div>
...
$(document).ready(function() {
    $('#addModal').on('show.bs.modal', function(e) { if(!$(e.relatedTarget).hasClass('edit-btn')){ $('#dataForm')[0].reset();$('#edit_id').val('');$('#modalTitle').html('<i class="fas fa-plus-circle me-2"></i>Add New Employee'); } });
    $(document).on('click', '.edit-btn', function() { const b=$(this);$('#edit_id').val(b.data('id'));$('#emp_id').val(b.data('eid'));$('#name_en').val(b.data('nameen'));$('#name_bn').val(b.data('namebn'));$('#father').val(b.data('father'));$('#mother').val(b.data('mother'));$('#dob').val(b.data('dob'));$('#joining').val(b.data('join'));$('#mobile').val(b.data('mobile'));$('#address').val(b.data('address'));$('#nid').val(b.data('nid'));$('#guarantor').val(b.data('guarantor'));$('#salary').val(b.data('salary'));$('#remarks').val(b.data('remarks'));$('#is_active').val(b.data('status'));$('#modalTitle').html('<i class="fas fa-edit me-2"></i>Edit Employee');$('#addModal').modal('show'); });
