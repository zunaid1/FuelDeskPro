...
<div class="table-container">
    <div class="table-header">
        <h5><i class="fas fa-user-friends text-primary me-2"></i><?php echo t('Customer Management'); ?></h5>
    ...
    <table class="table table-hover datatable">
        ...
        <tbody>
            ...
            <tr>
                <td><?php echo $sl++; ?></td>
                <td>
                    <span class="badge bg-light text-dark border me-1"><?php echo htmlspecialchars(!empty($row->TitleEN) ? $row->TitleEN : 'Mr.'); ?></span>
                    <span class="badge bg-light text-dark border"><?php echo htmlspecialchars(!empty($row->TitleBN) ? $row->TitleBN : 'জনাব'); ?></span>
                </td>
                <td class="fw-bold"><?php echo htmlspecialchars($row->CustomerName); ?></td>
                <td><?php echo htmlspecialchars($row->Mobile); ?></td>
                <td><?php echo htmlspecialchars($row->VehicleNumber ?? 'N/A'); ?></td>
                ...
                <td>
                    <button class="btn btn-sm btn-info edit-btn" 
                        ...
                        data-title-en="<?php echo htmlspecialchars($row->TitleEN ?? ''); ?>"
                        data-title-bn="<?php echo htmlspecialchars($row->TitleBN ?? ''); ?>"
                        data-name="<?php echo htmlspecialchars($row->CustomerName); ?>" 
                        data-mobile="<?php echo htmlspecialchars($row->Mobile); ?>" 
                        data-vehicle="<?php echo htmlspecialchars($row->VehicleNumber ?? ''); ?>" 
                        data-address="<?php echo htmlspecialchars($row->Address ?? ''); ?>" 
                        data-due="<?php echo $row->OpeningDue; ?>" 
                        data-status="<?php echo $row->IsActive; ?>">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger delete-btn" data-id="<?php echo $row->CustomerID; ?>">
        ...
        <div class="modal-content">
            <form id="dataForm" method="POST" action="../../modules/entry/customer_entry.php">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle"><i class="fas fa-plus-circle me-2"></i>Add New Customer</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="title_en" id="title_en" class="form-control" placeholder="e.g. Mr., Ms., Dr.">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Title (BN)</label>
                            <input type="text" name="title_bn" id="title_bn" class="form-control" placeholder="যেমন: জনাব, বেগম">
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" name="opening_due" id="opening_due" class="form-control" value="0">
                        </div>
                    </div>
                    <div class="mb-3">
...
$(document).ready(function() {
    $('#addModal').on('show.bs.modal', function(e) {
        if(!$(e.relatedTarget).hasClass('edit-btn')){
            $('#dataForm')[0].reset();
            $('#edit_id').val('');
            $('#modalTitle').html('<i class="fas fa-plus-circle me-2"></i>Add New Customer');
        }
    });
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        $('#title_en').val(b.data('title-en'));
        $('#title_bn').val(b.data('title-bn'));
        $('#customer_name').val(b.data('name'));
        $('#mobile').val(b.data('mobile'));
        $('#vehicle_no').val(b.data('vehicle'));
        $('#address').val(b.data('address'));
        $('#opening_due').val(b.data('due'));
        $('#is_active').val(b.data('status'));
        $('#modalTitle').html('<i class="fas fa-edit me-2"></i>Edit Customer');
        $('#addModal').modal('show');
    });
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        $.ajax({
            url:'../../modules/entry/customer_entry.php',
            type:'POST',
