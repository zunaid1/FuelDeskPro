...
$data = $objQuery->index("SELECT e.*, COALESCE(ep.ParticularNameEN, emp.NameEN, sh.NameEN, e.ParticularID) AS ParticularNameEN, COALESCE(ep.ParticularNameBN, emp.NameBN, sh.NameBN, emp.NameEN, sh.NameEN, e.ParticularID) AS ParticularNameBN, COALESCE(ec.ExpenseCategoryID, ec_sal.ExpenseCategoryID, ec_sh.ExpenseCategoryID) AS ExpenseCategoryID, COALESCE(ec.CategoryNameEN, ec_sal.CategoryNameEN, ec_sh.CategoryNameEN) AS CategoryNameEN, pm.MethodName FROM trx_expense e LEFT JOIN mst_expenseparticular ep ON (e.ParticularID=ep.ParticularID OR e.ParticularID=CAST(ep.ExpenseParticularID AS CHAR)) AND ep.IsDeleted=0 LEFT JOIN mst_expensecategory ec ON ep.ExpenseCategoryID=ec.ExpenseCategoryID AND ec.IsDeleted=0 LEFT JOIN mst_employee emp ON (e.ParticularID=emp.EmployeeId OR e.ParticularID=CAST(emp.Id AS CHAR)) AND emp.IsDeleted=0 LEFT JOIN mst_expensecategory ec_sal ON emp.ExpenseCategoryID=ec_sal.ExpenseCategoryID AND ec_sal.IsDeleted=0 LEFT JOIN mst_shareholder sh ON (e.ParticularID=sh.ShareHolderID OR e.ParticularID=CAST(sh.Id AS CHAR)) AND sh.IsDeleted=0 LEFT JOIN mst_expensecategory ec_sh ON sh.ExpenseCategoryID=ec_sh.ExpenseCategoryID AND ec_sh.IsDeleted=0 LEFT JOIN cfg_paymentmethod pm ON e.PaymentMethodID=pm.PaymentMethodID WHERE e.IsDeleted=0 ORDER BY e.ExpenseDate DESC, e.ExpenseID DESC");
?>
